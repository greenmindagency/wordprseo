            ...
            <div id="review_form_wrapper" class="mt-5">
                <div id="review_form" class="product-review-form card border-0 shadow-sm bg-white rounded-4">
                    <div class="card-body p-4 p-lg-5 bg-light">
                        ...
                        $error_notices = isset( $notices['error'] ) ? $notices['error'] : array();
                        ...
                        if ( ! empty( $error_notices ) ) {
                            ...
                            foreach ( $error_notices as $notice ) {
                                ...
                                echo '<p class="mb-1">' . esc_html( $notice['notice'] ) . '</p>';
                            }
                            echo '</div>';
                            ...
                            if ( function_exists( 'wc_set_notices' ) ) {
                                ...
                                if ( function_exists( 'wc_add_notice' ) ) {
                                    foreach ( $notices as $type => $type_notices ) {
                                        foreach ( $type_notices as $type_notice ) {
                                            if ( empty( $type_notice['notice'] ) ) {
                                                continue;
                                            }

                                            wc_add_notice( $type_notice['notice'], $type );
                                        }
                                    }
                                }
                            }
                        ...
                        if ( wc_review_ratings_enabled() ) {
                            ob_start();
                            $rating_group_name = 'rating';
                            $rating_group_id   = 'rating-group-' . ( $product ? $product->get_id() : 'product' );
                            ?>
                            <div class="mb-3">
                                <span id="rating-label" class="form-label d-block fw-semibold mb-2"><?php esc_html_e( 'Your rating', 'woocommerce' ); ?></span>
                                <div id="<?php echo esc_attr( $rating_group_id ); ?>" class="star-rating-input" role="radiogroup" aria-labelledby="rating-label">
                                    <?php for ( $rating_value = 5; $rating_value >= 1; $rating_value-- ) :
                                        ...
                                        $input_id      = $rating_group_id . '-' . $rating_value;
                                        $is_required   = ( 1 === $rating_value );
                                        ?>
                                        <input
                                            type="radio"
                                            id="<?php echo esc_attr( $input_id ); ?>"
                                            name="<?php echo esc_attr( $rating_group_name ); ?>"
                ...
                <div class="col-lg-7 col-md-8">
                    <div class="p-4 p-lg-5 h-100 d-flex flex-column justify-content-between">
                        <div>
                            ...
                            <div class="product-review-card__content text-muted fs-5">
                                <?php comment_text(); ?>
                            </div>
